...
use crate::apis::{ErrorHandler, query::*, schema::*, metadata::*};
use crate::models::{self, MultipleProblemsError, MultipleProblemsErrorProblemsInner, ShallowQuery, SystemInformation};
use crate::consts::{AUTHOR, VERSION, API_VERSION};
            ...
            if let Some(row_limit) = row_limit {
                let columns = columns.iter().map(|col| match col {
                    query_manager::Column::String(c) => {
                        ...
                        let column: Vec<String> = c.iter().flatten().flatten().take(row_limit).cloned().collect();
                    ...
                    query_manager::Column::Int64(c) => {
                        let column: Vec<i64> = c.iter().flatten().flatten().take(row_limit).cloned().collect();
                        assert!(row_count.is_none() || row_count.unwrap() == column.len());
                        row_count = Some(column.len());
                ...
                let columns = columns.iter().map(|col| match col {
                    query_manager::Column::String(c) => {
                        let column: Vec<String> = c.iter().flatten().flatten().cloned().collect();
                        assert!(row_count.is_none() || row_count.unwrap() == column.len());
                        row_count = Some(column.len());
                        models::QueryResultInnerColumnsInner::VecOfString(
                            Arc::new(column)
                        )
                    },
                    query_manager::Column::Int64(c) => {
                        ...
                        assert!(row_count.is_none() || row_count.unwrap() == column.len());
                        row_count = Some(column.len());
                        models::QueryResultInnerColumnsInner::VecOfi64(
...
impl QueryApi<ApiError> for ApiImpl {
    ...
    async fn submit_query(
        ...
        let state = self.query_manager.submit_query(query)
...
impl SchemaApi<ApiError> for ApiImpl {
    async fn create_table(
        ...
        Ok(match self.schema_manager.create_table(body.name.clone(), &columns).await {
            Ok(_) =>
                CreateTableResponse::Status200(body.name.clone()),
            Err(err) =>
                CreateTableResponse::Status400(
    ...
    async fn delete_table(
        ...
        Ok(match self.schema_manager.delete_table(&path_params.table_id).await {
            Ok(_) =>
                DeleteTableResponse::Status200,
            Err(err) =>
                DeleteTableResponse::Status404(models::Error {
    ...
    async fn get_table_by_id(
        ...
        Ok(match self.schema_manager.get_table_manager(&path_params.table_id).await {
            Some(table_manager) => {
                ...
                let columns = table.table.columns().iter()
                    .map(|c| models::Column {
                        name: c.name.clone(),
    ...
    async fn get_tables(
        ...
        let tables = self.schema_manager.list_tables().await
            .iter()
