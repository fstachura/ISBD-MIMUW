...
int main(int argc, char** argv) {
    ...
    struct timespec diff = {
        .tv_sec     = params.end.tv_sec - params.begin.tv_sec,
        .tv_nsec    = params.end.tv_nsec - params.begin.tv_nsec,
    };
    ...
    long long total = diff.tv_sec * 1000000000 + diff.tv_nsec;

    printf("file_size %ld, ", file_size);
    printf("bytes_read %ld, ", params.bytes_read);
    printf("block_size %ld, ", block_size);
    printf("blocks %ld, ", blocks);
    printf("hash %lx, ", params.hash);
    printf("time_ns %lld\n", total);
